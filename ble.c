...
static uint8_t time[2] = {0x00, 0x00};

//this code is based on the temperature measure algorithm in the soc_thermometer example from Simplicity Studio
static void __attribute__((optimize("O0"))) update_acceleration(float temp)
{
	uint8_t tempBuffer[4]; /* Stores the temperature data in the Health Thermometer (HTM) format. */
	uint8_t flags = 0x00;   /* HTM flags set as 0 for Celsius, no time stamp and no temperature type. */
	uint32_t accel;   /* Stores the temperature data read from the sensor in the correct format */
	uint8_t *p = tempBuffer; /* Pointer to HTM temperature buffer needed for converting values to bitstream. */


	/* Convert sensor data to correct temperature format */
	accel = FLT_TO_UINT32(abs(temp), 0);
	/* Convert temperature to bitstream and place it in the HTM temperature data buffer (htmTempBuffer) */
	UINT32_TO_BITSTREAM(p, accel);

	/* Send indication of the temperature in htmTempBuffer to all "listening" clients.
	 * This enables the Health Thermometer in the Blue Gecko app to display the temperature.
	 *  0xFF as connection ID will send indications to all connections. */
	BTSTACK_CHECK_RESPONSE(gecko_cmd_gatt_server_write_attribute_value(gattdb_accel_read,0, 4, tempBuffer));
}
void __attribute__((optimize("O0"))) handle_ble_event(struct gecko_cmd_packet* evt, uint8_t mode)
{
...
//	            gecko_cmd_hardware_set_soft_timer(32768, 0, 0);
	      ...
	      case gecko_evt_le_connection_opened_id:
	    	...
	    	BTSTACK_CHECK_RESPONSE(gecko_cmd_le_connection_set_parameters(connection, 60, 60, 4, 500));
//	    	gecko_cmd_hardware_set_soft_timer(3277, 0, 0);
	      ...
	      case gecko_evt_system_external_signal_id:
	    	  ...
	    	  if(evt->data.evt_system_external_signal.extsignals & LETIMER0_UF_EVENT)
	    	  {
	    		  ...
	    		  count++;
				  time[1] = (((uint16_t)count*(LETIMER_PERIOD_MS/1000)) >> 0) & 0xFF;
				  time[0] = (((uint16_t)count*(LETIMER_PERIOD_MS/1000)) >> 8) & 0xFF;
	    		//accel = getAccel();
	    		  ...
	    		  {
	    			  BTSTACK_CHECK_RESPONSE(gecko_cmd_gatt_server_write_attribute_value(gattdb_jump_height,0, 2, time));
	    		  }
	    		  else if(mode == 2)
	    		  {
	    			  BTSTACK_CHECK_RESPONSE(gecko_cmd_gatt_server_write_attribute_value(gattdb_accel_read,0, 2, time));
	    		  }
	    		  else if(mode == 3)
	    		  {
	    			  BTSTACK_CHECK_RESPONSE(gecko_cmd_gatt_server_write_attribute_value(gattdb_lift_reps,0, 2, time));
	    		  }
	    	  }
	    	  break;
